...
### سوال: وضعیت سیستم از نظر اصل OCP چه تغییری کرده است؟ کجاها بهتر و کجاها پیچیده‌تر شده است؟
اصل OCP (Open/Closed Principle) می‌گوید نرم‌افزار باید برای توسعه باز و برای تغییر بسته باشد.

۱. کجا بهتر شده است؟ 

در کد اولیه، سیستم به شدت این اصل را نقض می‌کرد. مثلاً اگر می‌خواستیم وضعیت جدیدی به نام "Suspended" اضافه کنیم، مجبور بودیم کل کد کلاس TicketService را باز کنیم و یک if جدید وسط آن بنویسیم (یعنی تغییر در کد قبلی)؛

اما در طراحی جدید:
.سیستم نسبت به تغییر بسته است؛ یعنی به کدهای قبلی دست نمی‌زنیم.

.سیستم نسبت به توسعه باز است؛ یعنی فقط یک کلاس جدید (مثلاً SuspendedState) اضافه می‌کنیم و تمام! هیچ جای دیگری از برنامه خراب نمی‌شود.

۲. کجا پیچیده‌تر شده است؟ 

تعداد فایل‌ها: به جای یک کلاس، حالا چندین اینترفیس و کلاس داریم که مدیریت فایل‌ها را کمی بیشتر می‌کند.

دنبال کردن کد (Trace): برای یک برنامه نویس تازه‌کار، فهمیدن اینکه تیکت چطور بین کلاس‌های مختلف جابجا می‌شود، در نگاه اول کمی سخت‌تر از خواندن یک if ساده است.
